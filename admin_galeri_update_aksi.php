				...
				<?php
					...
					if($movie){
							$id_galery		=	$_POST['id_galery'];
								$keterangan		= $_POST['judul'];
								$tgl_post = $_POST ['tgl_post'];
								$sts_post = $_POST ['sts_post'];
								 $simpan = mysqli_query($konek,"update tb_galery set gambar='$fileName',keterangan='$keterangan', tgl_post='$tgl_post', sts_post='$sts_post' where id_galery='$id_galery'");
					
						if($simpan){
						  ?>
						 <script type="text/javascript">
							alert("sukses");window.location='admin/galeri.php'
						</script>
						<?php
					} else {
						?>
